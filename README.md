...
### 구현 방향
- '선물하기'의 목적에 맞게 주문 생성과 메시지 전송을 하나의 트랜잭션으로 구성한다.[O]
- 이외 상품 옵션 차감과 위시 삭제 로직을 주문 도메인을 구성하여 구현한다. [O]
- Token entity를 구성하여 DB에 저장하도록 구현한다. [X]
- Access Token은 평문/ Refresh Token은 암호화하여 저장하며 만료 시 자동 갱신 로직을 구현한다. [X]
